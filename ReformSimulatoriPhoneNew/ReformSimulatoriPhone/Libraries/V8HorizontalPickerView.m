...
//added by yamada
- (V8HorizontalPickerImageView *)imageForElementAtIndex:(NSInteger)index withTitle:(NSString *)title;
...
- (id)initWithFrame:(CGRect)frame {
	if ((self = [super initWithFrame:frame])) {
		...
		numberOfElements     = 0;
		elementPadding       = 100; // fixed by yamada
...
- (void)layoutSubviews {
    ...
    SEL imageForElementSelector  = @selector(horizontalPickerView:imageForElementAtIndex:);
	...
	for (int i = firstNeededElement; i <= lastNeededElement; i++) {
		...
		if (!view) {
			if (i < numberOfElements) { // make sure we are not requesting data out of range
				...
				} else if (self.delegate && [self.delegate respondsToSelector:imageForElementSelector]) {
                    NSString *title = [self.delegate horizontalPickerView:self imageForElementAtIndex:i];
                    view = [self imageForElementAtIndex:i withTitle:title];
                }
...
- (void)addScrollView {
	if (_scrollView == nil) {
		...
		_scrollView.minimumZoomScale = 0.7; // setting min/max the same disables zooming //fixed by yamada
		_scrollView.maximumZoomScale = 1.3; //fixed by yamada
...
// added by yamada //create a UIImage for this element.
- (V8HorizontalPickerImageView *)imageForElementAtIndex:(NSInteger)index withTitle:(NSString *)title {
	CGRect imageFrame     = [self frameForElementAtIndex:index];
	elementImageView = [[V8HorizontalPickerImageView alloc] initWithFrame:imageFrame];
    
	elementImageView.image = [UIImage imageNamed:title];
    
	// show selected status if this element is the selected one and is currently over selectionPoint
	int currentIndex = [self nearestElementToCenter];
	elementImageView.selectedElement = (currentSelectedIndex == index) && (currentIndex == currentSelectedIndex);
...
// ------------------------------------------------------------------------
#pragma mark - Picker Image Implementation
@implementation V8HorizontalPickerImageView : UIImageView
